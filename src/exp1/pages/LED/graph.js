...
function drawAxis() {
  ...
  canvas_arrow(graphRepContext, 100, 550, 550, 550);
  canvas_arrow(graphRepContext, 100, 550, 550, 100);
...
function drawLabels() {
  ...
  graphRepContext.fillText("P-I Characteristics of LED", 220, 60);
...
function makePlots(x, y, valueX, valueY) {
  ...
  graphRepContext.fillText(`( ${valueX}, ${valueY} )`, x, y);
  graphRepContext.stroke();
  graphRepContext.closePath();
}

function makeSinglePlot(x, y, value) {
  graphRepContext.beginPath();
  graphRepContext.moveTo(x, y);
...
function updatePlot() {
  ...
  for (var i = 0; i < length - 1; i++) {
    graphRepContext.moveTo(len + 100, 550 - len);
    graphRepContext.arc(len + 100, 550 - len, 2, 0, 2 * Math.PI, true);
    graphRepContext.moveTo(len + 100, 550);
    graphRepContext.arc(len + 100, 550, 2, 0, 2 * Math.PI, true);
    graphRepContext.moveTo(100, 550 - len);
    graphRepContext.arc(100, 550 - len, 2, 0, 2 * Math.PI, true);
    graphRepContext.stroke();
    makePlots(len + 120, 550 - len, xAxisBuffer[i], yAxisBuffer[i]);
    makeSinglePlot(len + 95, 580, xAxisBuffer[i]);
    makeSinglePlot(70, 555 - len, yAxisBuffer[i]);
    len = len + minLen;
  }
  if (length <= 15) {
    xAxisBuffer.push(xAxisBuffer[length - 1] + 2.0);
    yAxisBuffer.push(yAxisBuffer[length - 1] + 0.20);
  }
}
