class Component {
    constructor(name, startX, startY, endX, endY, len) {
        this.name = name;
        this.startX = startX;
        this.startY = startY;
        this.endX = endX;
...
class Circuit {
    ...
    power(len=50, n=2, label="") {
      ...
      while (n--) {
          this.ctx.moveTo(wl+space*(2*n+1), -2*space);
          this.ctx.lineTo(wl+space*(2*n+1), 2*space);
          this.ctx.moveTo(wl+space*(2*n), -space);
          this.ctx.lineTo(wl+space*(2*n), space);
      }
